...
function SideBarPage({ className }) {
        ...
        <div className={`side-bar__wrapper 
            ...
            <SideBarContext.Provider value={{ isExpand, onSetExpand: setExpand }}>
                ...
                <ul className='side-bar__home'>
                    {teacherComponents.map((item, index) => <SideBarItem key={index} item={item} isExpand={isExpand} onActive={setActiveIndex} activeIndex={activeIndex} />)}
                </ul>

                <ul className='side-bar__study-info'>
                    {teacherMiddleComponents.map((item, index) => <SideBarItem key={index} item={item} isExpand={isExpand} onActive={setActiveIndex} activeIndex={activeIndex} />)}
                </ul>

                <ul className='side-bar__extension'>
                    {teacherLastComponents.map((item, index) => <SideBarItem key={index} item={item} isExpand={isExpand} onActive={setActiveIndex} activeIndex={activeIndex} />)}
                </ul>
