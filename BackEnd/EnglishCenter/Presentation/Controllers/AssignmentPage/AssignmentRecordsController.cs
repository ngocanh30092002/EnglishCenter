...
namespace EnglishCenter.Presentation.Controllers.AssignmentPage
{
    ...
    {
        private readonly IAssignmentRecordService _answerService;

        public AssignmentRecordsController(IAssignmentRecordService answerService)
        {
        ...
        {
            var response = await _answerService.GetResultAsync(processId);
            return await response.ChangeActionAsync();
        }

        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] AssignRecordDto model)
        {
        ...
        public async Task<IActionResult> UpdateAsync([FromRoute] long id, [FromForm] AssignRecordDto model)
        {
